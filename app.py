...
def members():
    if request.method == 'POST':
        try:
            ...
            success = member_service.add_member(
    ...
    try:
        members_list = member_service.get_all_members()
...
def events():
    if request.method == 'POST':
        try:
            ...
            success = event_service.add_event(name, description, date, organizer)
    ...
    try:
        events_list = event_service.get_all_events()
...
def subscriptions():
    if request.method == 'POST':
        try:
            ...
            if member:
                ...
                success = subscription_service.add_subscription(member.id, amount, date, status)
    ...
    try:
        subscriptions_list = subscription_service.get_all_subscriptions()
        members_list = member_service.get_all_members()
